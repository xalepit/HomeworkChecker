...
private:
	...
	/* ---------------------------------------------------------------
	   ...
	   ---------------------------------------------------------------- */
	void init(const char* name, const enum ST_EXTARGS_TYPE type, const int ext_num);
	void print_extarg_is_missing(const bool extarg_exist, const bool extarg_is_param) const;
	void print_annotation() const;

	string ipdef_to_str() const;

	bool parse_extarg(const string extarg);
	bool parse_int_extarg(const string extarg);
	bool parse_double_extarg(const string extarg);
	bool parse_strset_extarg(const string extarg);
	bool parse_ipaddr_extarg(const string extarg);


	int extargs_intset_length;
	int extargs_doubleset_length;
	int extargs_stringset_length;
	char pad[56];
