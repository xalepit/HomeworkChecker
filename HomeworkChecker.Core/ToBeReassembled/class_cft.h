...
private:
	...
	struct cfg_item {
		string raw_line;   //该行去掉注释且trim后的原始内容
		string item_name;  //（按分隔符分割且trim后的）有效项名
		string item_value; //（按分隔符分割且trim后的）有效项值
	};

	struct cfg_group {
		string group_name; //组名，以[***]的形式存储
		vector<cfg_item> items; //该组下的所有配置项
	};

	vector<cfg_group> groups; //配置文件中的所有组
	bool read_succeeded; //标记配置文件是否成功读取

	string remove_comment(const string& line); //去掉注释
	void trim(string& str); //去掉字符串前后的空格和tab
	bool is_group(const string& str); //判断某行是否为组
	string get_valid_group_name(const string& str); //取组名
	void parse_item_line(const string& valid_line, cfg_item& item); //解析配置项行，取出项名、项值、是否含分隔符
	int find_group(const string& target_group_name, const bool is_case_sensitive = true) const; //查找组，返回组在groups中的索引，找不到返回-1
	int find_item(const int group_index, const string& target_item_name, const bool is_case_sensitive = true) const; //查找项，返回项在items中的索引，找不到返回-1

public:
	/* 构造函数的作用：将指定配置文件中的所有组/项均读入并存储到自定义结构中，方便后续各种get函数的调用 */
	config_file_tools(const char* const _cfgname, const enum BREAK_CTYPE _ctype = BREAK_CTYPE::Equal);
	config_file_tools(const string& _cfgname, const enum BREAK_CTYPE _ctype = BREAK_CTYPE::Equal);
